// Decrypto.cpp : Defines the entry point for the console application.
//
...
int main(int argc, char* argv[])
{
	...
	printf("Enter the name of the file to be decrypted: ");//输入要解密的源文件名
	scanf("%s",szSource);
	printf("Enter the name of the output file: ");//解密后文件输出存放地址
	scanf("%s",szDestination);
	printf("Enter the password:");//解密密码
	scanf("%s",szPassword);
...
{ 		
	...
	{ 		
		...
		if(!CryptDecrypt(
					hKey,			//密钥
					0,				//如果数据同时进行散列和解密，这里传入一个散列对象
					feof(hSource),	//如果是最后一个被解密的块，输入TRUE.如果不是输.
									//入FALSE这里通过判断是否到文件尾来决定是否为最后一块。
					0,				//保留
					pbBuffer,		//输入被解密数据，输出解密后的数据
					&dwCount))		//输入被解密数据实际长度，输出解密后数据长度
		{ 
